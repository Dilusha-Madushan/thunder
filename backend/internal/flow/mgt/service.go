...
// An optional flowType argument can be provided to also validate that the flow
// matches the expected type. If no flowType is provided, only the existence of
// the flow is checked. Only the first flowType value is used if multiple are provided.
// Returns (false, nil) when the flow is not found or type mismatches (client error).
...
func (s *flowMgtService) IsValidFlow(
	...
	if len(flowType) > 0 {
		return flow.FlowType == flowType[0], nil
	}
