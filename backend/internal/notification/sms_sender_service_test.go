...
type SMSSenderServiceTestSuite struct {
	suite.Suite
...
func TestSMSSenderServiceTestSuite(t *testing.T) {
	suite.Run(t, new(SMSSenderServiceTestSuite))
}

func (suite *SMSSenderServiceTestSuite) SetupSuite() {
...
func (suite *SMSSenderServiceTestSuite) SetupTest() {
	...
	suite.service = &smsSenderService{
		senderMgtService: suite.mockSenderMgtSvc,
		clientProvider:   suite.mockClientProvider,
	}
}

func (suite *SMSSenderServiceTestSuite) getValidSender() *common.NotificationSenderDTO {
...
func (suite *SMSSenderServiceTestSuite) TestSendSMS_Success() {
	...
	suite.mockClientProvider.EXPECT().GetMessageClient(mock.Anything).Return(mm, nil).Once()

	err := suite.service.SendSMS(context.Background(), "sender-001", "+94714627887", "Test message")
	suite.Nil(err)
}

func (suite *SMSSenderServiceTestSuite) TestSendSMS_GetSenderError() {
...
func (suite *SMSSenderServiceTestSuite) TestSendSMS_WrongSenderType() {
	...
	err := suite.service.SendSMS(context.Background(), "email-sender-001", "+94714627887", "Test message")
	suite.NotNil(err)
	suite.Equal(ErrorRequestedSenderIsNotOfExpectedType.Code, err.Code)
}

func (suite *SMSSenderServiceTestSuite) TestSendSMS_GetMessageClientError() {
	sender := suite.getValidSender()
	suite.mockSenderMgtSvc.On("GetSender", mock.Anything, "sender-001").Return(sender, nil).Once()
	suite.mockClientProvider.EXPECT().GetMessageClient(mock.Anything).
...
func (suite *SMSSenderServiceTestSuite) TestSendSMS_ClientSendError() {
	sender := suite.getValidSender()
	suite.mockSenderMgtSvc.On("GetSender", mock.Anything, "sender-001").Return(sender, nil).Once()

	mm := messagemock.NewMessageClientInterfaceMock(suite.T())
	mm.EXPECT().SendSMS(mock.Anything).Return(errors.New("network error")).Once()
	suite.mockClientProvider.EXPECT().GetMessageClient(mock.Anything).Return(mm, nil).Once()
