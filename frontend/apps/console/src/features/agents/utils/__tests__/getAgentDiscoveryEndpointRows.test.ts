...
import getAgentDiscoveryEndpointRows from '../getAgentDiscoveryEndpointRows';

const identityT = (_key: string, fallback: string): string => fallback;

describe('getAgentDiscoveryEndpointRows', () => {
  it('returns the curated rows derived from a fully populated wellKnown document', () => {
    const rows = getAgentDiscoveryEndpointRows(
      {
  ...
  it('omits the issuer-derived discovery row when the issuer is missing', () => {
    const rows = getAgentDiscoveryEndpointRows({token_endpoint: 'https://localhost:8090/oauth2/token'}, identityT);

    expect(rows.map((row) => row.key)).toEqual(['token']);
  });

  it('returns an empty array when wellKnown is undefined', () => {
    expect(getAgentDiscoveryEndpointRows(undefined, identityT)).toEqual([]);
  });

  it('returns an empty array when wellKnown is null', () => {
    expect(getAgentDiscoveryEndpointRows(null, identityT)).toEqual([]);
  });

  it('uses the translation function to resolve each row label', () => {
    const t = (key: string): string => `translated:${key}`;

    const rows = getAgentDiscoveryEndpointRows({issuer: 'https://localhost:8090'}, t);

    expect(rows.find((row) => row.key === 'issuer')?.label).toBe('translated:agents:clientSecret.endpoints.issuer');
  });
});
